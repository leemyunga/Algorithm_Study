...
정보는 어떠한 목적이나 의도에 맞게 데이터를 가공처리한 것입니다. 데이터는 가공 전의 값을 이야기합니다.
...
[A] 
데이터베이스를 관리하는 시스템인 DBMS 는 데이터베이스를 구축하는 틀을 제공하고 검색 및 저장하는 기능을 제공해줍니다. 또한 데이터베이스를 공유하고 사용할 수 있는 환경을 제공해줍니다.
...
[A] 
RDBMS는 관계형 데이터베이스를 관리하는 시스템입니다. 데이터를 2차원 테이블 형태로 표현해주고 관계를 통한 테이블간의 연결을 통해 사용할 수 있습니다.
...
[A] 
NoSQL 비관계형 데이터 베이스로 관계가 존재하지않습니다. RDBMS 와는 달리 많은 양의 데이터를 다루는 것에 특화되어 있습니다. 스키마 없이 사용이 가능해서 좀 더 자유롭게 데이터를 관리할 수 있지만, 하지만 join이 불가능하고 수정이 많을 경우에는 적합하지 않습니다.

>> RDBMS 처럼 여러 테이블에 나누어 담지 않고, 관련 데이터를 동일한 컬렉션에 넣음. 그래서 관계가 없기 때문에 JOIN 이란 개념이 존재하지않음
컬렉션 : 그저 용도가 같거나 유사한 문서들을 그룹으로 묶은 것을 말한다. 
<br>

### [Q] SQL(Structured Query Language)란?
[A] 
sql 은 데이터베이스에서 사용하는 쿼리 언어입니다. sql을 이용해 RDBMS 에서 데이터를 관리 할 수 있는데. 이 때 데이터 정의어(DDL), 조작어(DML), 제어어(DCL)로 나눌 수 있습니다

DDL (Data Defined Language : 데이터 정의어) : CREATE, DROP, ALTER, RENAME
...
[A] 
우선 DROP 은 테이블 자체를 완전히 삭제하는 것이고, 로그도 남지않기 때문에 Rollback 이 불가능합니다. DELETE는 테이블안 특정 행의 데이터만 삭제하는 것입니다. 마지막으로 TRUNCATE 는 테이블 내의 데이터를 모두 삭제하는 것입니다.
<br>
...
### [Q] 테이블 간 관계에 대해 설명
[A] 
테이블간의 관계는 외래키를 통해  수 있습니다. 종류로는 1:1, 1:N, M:1, M:N 이 있습니다.
<br>

### [Q] JOIN 종류 및 설명
[A] 
JOIN 은 공통된 컬럼이 존재하는 둘 이상의 테이블을 연결해서 데이터를 검색하는 방법입니다. 크게 교집합으로 중복된 값만 보여주는  inner join과 한 쪽 테이블에만 값이 존재해도 보여주는 outer join이 있습니다. 
...
>> 배타락 : 데이터를 수정할 때 하나의 트랜잭션이 완료될 때까지는 접근을 막음
>> 공유락 : 데이터를 읽을 때 사용하고, 다른 사용자가 읽을 수는 있지만 변경은 안 됨

<br>

### [Q] 교착상태(Deadlock)란?
[A] 
복수의 트랜잭션을 사용하다보면 교착상태가 일어날 수 있습니다. 여러 개의 작업들이 서로의 작업이 끝나기를 기다리면서 멈춰있는 상황을 말합니다.  작업이 다른 작업이 사용중인 자원을 기다리는 동안은 진행을 할 수 없는 상태가 됩니다. 이 상황을 교착상태라고 합니다.
<br>
...
### [Q] 정규화란?
[A] 
정규화는 관계형 데이터베이스에서 이상현상을 최소화 하기 위해 데이터를 구조화하는 작업입니다. 이를 통해 데이터의 일관성과 정확성을 보장할 수 있습니다. 


>>1차 정규화는 null 값이 발생하지 않도록 테이블을 분리하는 작업입니다. 
 2차 정규화는 가급적이면 하나의 키로 구분할 수 있도록 테이블을 구성하는 작업입니다. 
 3차 정규화는 2차 정규형을 만족하고, 이행 함수 종속성이라고 a로 인해 b가 결정되고, b로 인해 c가 결정되는 경우에 a 가 c로 성립되는 것을 못하도록 분해하는 작업입니다.
<br>
...
### [Q] Mybatis vs JPA
[A] 
MyBatis와 JPA는 데이터베이스와 Java 객체 간의 상호작용을 다루는 기술입니다.
MyBatis는 Object Mapping 기술을 사용해서 Java 클래스 코드와 SQL 코드를 직접 매핑해야 한다는 것을 의미합니다. 
반면 JPA 는 객체와 데이터베이스 간의 연결을 자동으로 처리해기 때문에 SQL 을 작성할 필요가 없습니다.
<br>

### [Q] JDBC (Java Database Connectivity) vs ODBC (Open Database Connectivity)
[A] 
JDBC는 Java 언어를 위한 데이터베이스 연결 기술이고, ODBC는 다양한 프로그래밍 언어에서 데이터베이스와 통신하기 위한 표준 인터페이스입니다. JDBC는 Java 애플리케이션과 특정 데이터베이스 간의 통신을 담당하고, ODBC는 다양한 환경에서 데이터베이스에 접속할 수 있도록 도와줍니다.
<br>
